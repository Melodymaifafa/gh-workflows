...
- 定时挪的代价是 `main` 不再等于「我确认过这版能用」。**CI 闸门是那句话的自动替代品** —— develop 最新 commit 不全绿（或压根没跑过 CI）就跳过、推一条 Pushover，不硬挪。想无视它：手动跑一次，取消勾选 `require_green`。
...
| `ff-main.yml` | 每月 1 / 15 号 09:00，也可手动点 | 把 main 快进到 develop。CI 不全绿就跳过并推手机通知；分叉了直接拒绝 |
